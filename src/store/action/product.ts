...
import {fetchFlashProductApi, fetchLatestProductApi, fetchMostViewedApi} from "../../utility/apis";
import {
...
export const fetchMostViewed = () => async (dispatch: any) => {
  dispatch(flashStart());
  try {
    const response = await axios.get(fetchMostViewedApi);
    dispatch(flashSuccess(response.data));
  } catch (err) {
    console.log(err);
    dispatch(flashError(err));
